...
/*        	var mid=$('#mid').val();
                $.ajax({
                    async : true,
                    type : 'POST',
                    data : mid,//mid라는 이름으로 mid라는 데이터를 @WebServlet("/idsearch.do")에 보내겠다
                    url : 'idcheck.do',
                    dateType: 'json',
...
*/        }//else if
    ...
    $('#pwd').blur(function() {
        if (pwJ.test($('#pwd').val())) {
            ...
            console.log('false');
            $('#pwd_check').text('10자 이상 입력, 영문/숫자/특수문자(공백 제외)만 허용하며, 2개 이상 조합');
    ...
    $('#pwdCheck').blur(function() {
    	if ($('#pwd').val() != $(this).val()) {
    		$('#pwd_Recheck').text('동일한 비밀번호로 입력해주세요.');
    		$('#pwd_Recheck').css('color', 'red');
    	} else {
    		$('#pwd_Recheck').text('');
    	}
    });
